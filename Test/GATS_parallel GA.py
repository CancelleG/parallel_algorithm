#状态:未完成
#来源：从GA_thread_TS_thread.py改进
#改进：可实现真正的GA并行，并且结果可以安全返回且终止其他进程
#错误：不能完成不同GA间的elite的交换导致收敛速度较慢
...
def mutate(individual, mutationRate, Tabu_table):       #变异，采用交换基因策略；每个基因都需要进行一定概率变异
    ...
    def SWAP(individual, part_no, part):
        ...
        for swapped in range(begin, end):
            ...
            # if (random.random() < mutationRate): #GATS_no_parallel 用0.5效果不错
...
def geneticAlgorithm(threadName, population, popSize, eliteSize, mutationRate, generations, exitProcessFlag, progress, bestRoute_forplot):
    ...
    while(exitProcessFlag.value):
        ...
        progress_sub.append(1 / rankRoutes(pop)[0][1])
        if int(1 / rankRoutes(pop)[0][1]) < 8000 or gener_count == generations and exitProcessFlag:      #通过brute_forces_tsp运行得出结果，11：4038;  52:7544
                                                        #数据#输入
...
def main():
    ...
    p1 = multiprocessing.Process(target=geneticAlgorithm, args=("process1", cityList, data_len, 5, 0.01, 50,exitProcessFlag, progress, bestRoute_forplot))
    p2 = multiprocessing.Process(target=geneticAlgorithm, args=("process2", cityList, data_len, 5, 0.01, 50,exitProcessFlag, progress, bestRoute_forplot))
    p3 = multiprocessing.Process(target=geneticAlgorithm, args=("process3", cityList, data_len, 5, 0.01, 50,exitProcessFlag, progress, bestRoute_forplot))
    p4 = multiprocessing.Process(target=geneticAlgorithm, args=("process4", cityList, data_len, 5, 0.01, 50,exitProcessFlag, progress, bestRoute_forplot))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
    ...
    bestRoute_forplot = cityList
    for gene_index in range(len(bestRoute_forplot)):
        bestRoute_forplot[gene_index] = cityList[bestRoute[gene_index]]
