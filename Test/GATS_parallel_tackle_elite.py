...
def mutate(individual, mutationRate, Tabu_table):       #变异，采用交换基因策略；每个基因都需要进行一定概率变异
    ...
    for swapped in range(len(individual)):
        ...
        if(random.random() < 0.5 * mutationRate):
...
def main():
    ...
    p1 = multiprocessing.Process(target=geneticAlgorithm,args=("process1", cityList, data_len, 5, 0.01, 500000,exitProcessFlag, progress, bestRoute_forplot, elite_all_proc,lock))
    p2 = multiprocessing.Process(target=geneticAlgorithm,args=("process2", cityList, data_len, 5, 0.01, 500000,exitProcessFlag, progress, bestRoute_forplot, elite_all_proc,lock))
    p3 = multiprocessing.Process(target=geneticAlgorithm,args=("process3", cityList, data_len, 5, 0.01, 500000,exitProcessFlag, progress, bestRoute_forplot, elite_all_proc,lock))
    p4 = multiprocessing.Process(target=geneticAlgorithm,args=("process4", cityList, data_len, 5, 0.01, 500000,exitProcessFlag, progress, bestRoute_forplot, elite_all_proc,lock))
    p1.start()
    p2.start()
    p3.start()
    p4.start()
